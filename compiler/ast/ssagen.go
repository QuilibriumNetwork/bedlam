...
func (ast *Return) SSA(block *ssa.Block, ctx *Codegen, gen *ssa.Generator) (
	*ssa.Block, []ssa.Value, error) {
	...
	for idx, r := range ctx.Func().Return {
		...
		// 	fmt.Println("ssa not lval")
		// 	fmt.Println(typeInfo.String())
		// 	fmt.Println(result[idx].String())
		// 	return nil, nil, ctx.Errorf(ast,
		// 		"invalid value %v for return value %v",
...
// SSA implements the compiler.ast.AST.SSA for composite values.
func (ast *CompositeLit) SSA(block *ssa.Block, ctx *Codegen,
	gen *ssa.Generator) (*ssa.Block, []ssa.Value, error) {
	typeInfo, err := ast.Type.Resolve(NewEnv(ctx.Start()), ctx, gen)
	if err != nil {
		return nil, nil, ctx.Errorf(ast.Type, "invalid argument type: %s", err)
	}
	if !typeInfo.Concrete() {
		return nil, nil,
			ctx.Errorf(
				ast,
				"struct %s is not concrete, cannot be used here",
				ast.Type.String(),
			)
	}
	...
	bits := 0
	for _, field := range typeInfo.Struct {
		bits += int(field.Type.Bits)
		t := gen.AnonVal(field.Type)
		values = append(values, t)
	}
	a := gen.NewVal(ast.Type.TypeName, typeInfo, ctx.Scope())

	return block, []ssa.Value{a}, nil
}
